...
export const MAIN_RUNTIME_POLICY = Object.freeze({
  ...
  commandAvailability: {
    ...
    initialIntervalMs: readEnvNumber(
      process.env,
      'CCCLAW_RUNTIME_COMMAND_AVAILABILITY_INITIAL_INTERVAL_MS',
      MAIN_RUNTIME_DEFAULTS.commandAvailability.initialIntervalMs,
      100
    ),
    maxIntervalMs: readEnvNumber(
      process.env,
      'CCCLAW_RUNTIME_COMMAND_AVAILABILITY_MAX_INTERVAL_MS',
  ...
  auth: {
    pluginEnableTimeoutMs: readEnvNumber(
      process.env,
      'CCCLAW_RUNTIME_AUTH_PLUGIN_TIMEOUT_MS',
    ...
    persistencePoll: {
      timeoutMs: readEnvNumber(
        process.env,
        'CCCLAW_RUNTIME_AUTH_PERSISTENCE_TIMEOUT_MS',
        MAIN_RUNTIME_DEFAULTS.auth.persistencePoll.timeoutMs,
        1_000
      ),
      initialIntervalMs: readEnvNumber(
        process.env,
        'CCCLAW_RUNTIME_AUTH_PERSISTENCE_INITIAL_INTERVAL_MS',
        MAIN_RUNTIME_DEFAULTS.auth.persistencePoll.initialIntervalMs,
        200
      ),
      maxIntervalMs: readEnvNumber(
        process.env,
        'CCCLAW_RUNTIME_AUTH_PERSISTENCE_MAX_INTERVAL_MS',
        MAIN_RUNTIME_DEFAULTS.auth.persistencePoll.maxIntervalMs,
        200
      ),
      backoffFactor: readEnvFloat(
        process.env,
        'CCCLAW_RUNTIME_AUTH_PERSISTENCE_BACKOFF_FACTOR',
  ...
  modelConfig: {
    actionTimeoutMs: readEnvNumber(
      process.env,
      'CCCLAW_RUNTIME_MODEL_CONFIG_TIMEOUT_MS',
      MAIN_RUNTIME_DEFAULTS.modelConfig.actionTimeoutMs,
      1_000
    ),
    statusTimeoutMs: readEnvNumber(
      process.env,
      'CCCLAW_RUNTIME_MODEL_STATUS_TIMEOUT_MS',
  ...
  modelCatalog: {
    fetchTimeoutMs: readEnvNumber(
      process.env,
      'CCCLAW_RUNTIME_MODEL_CATALOG_TIMEOUT_MS',
      MAIN_RUNTIME_DEFAULTS.modelCatalog.fetchTimeoutMs,
      1_000
    ),
    cacheTtlMs: readEnvNumber(
      process.env,
      'CCCLAW_RUNTIME_MODEL_CATALOG_TTL_MS',
  ...
  processControl: {
    cancelGracePeriodMs: readEnvNumber(
      process.env,
      'CCCLAW_RUNTIME_CANCEL_GRACE_MS',
